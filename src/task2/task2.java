...
public class task2 {
    public static void main(String[] args){
        try{
            takePath("src/task2/circlePoint.txt","src/task2/point.txt");
    ...
    static void checkIn(String circlePoint,String rad, String targetPoint){
        ...
        if(Math.pow((intArrOfTargetPoint[0] - intArrOfCirclePoint[0]),2) + Math.pow((intArrOfTargetPoint[1] - intArrOfCirclePoint[1]),2) > Math.pow(r,2)){
            System.out.println("2 - точка снаружи");
        }else if(Math.pow((intArrOfTargetPoint[0] - intArrOfCirclePoint[0]),2) + Math.pow((intArrOfTargetPoint[1] - intArrOfCirclePoint[1]),2) < Math.pow(r,2)){
